...
var contBtn=h('button',{id:'bugaputa-video-continue',type:'button',text:'Attach recording'}); contBtn.setAttribute('aria-label','Attach recording'); contBtn.title='Attach recording'; contBtn.style.minHeight='44px';
...
var vEl=document.createElement('video'); vEl.setAttribute('controls',''); vEl.setAttribute('playsinline',''); vEl.setAttribute('preload','metadata'); vEl.src=vUrl; vEl.style.cssText='max-width:100%;max-height:160px;border-radius:8px;border:1px solid #e2e8f0;background:#0f172a;display:block';
...
var vHint=h('div',{text:'Video attached — '+vMeta,style:'font-size:11px;color:#64748b;margin-top:6px'}); vHint.id='bugaputa-video-hint';
preview.appendChild(vHint);
var vActions=h('div',{style:'display:flex;gap:8px;margin-top:8px;flex-wrap:wrap'});
var vPreviewBtn=h('button',{type:'button',text:'Preview',id:'bugaputa-video-preview-again'}); vPreviewBtn.style.minHeight='44px'; vPreviewBtn.addEventListener('click', function(){ try{ if(vEl.paused) vEl.play(); else vEl.pause(); }catch(_){} });
var vReplaceBtn=h('button',{type:'button',text:'Record again',id:'bugaputa-video-replace'}); vReplaceBtn.style.minHeight='44px'; vReplaceBtn.addEventListener('click', function(){
var vp=document.getElementById('bugaputa-video-preview-modal'); if(vp) try{ vp.remove(); }catch(_){}
var fw=document.getElementById('bugaputa-form-wrap'); if(fw) fw.style.display='none';
var ov=document.getElementById('bugaputa-overlay');
if(window.__bugaputaVideoLive&&window.__bugaputaVideoLive.open){ try{ window.__bugaputaVideoLive.open({getDocPoint:getDocPoint, fmtVideoTime:fmtVideoTime, ensureHalo:ensureHalo, movePointerHalo:movePointerHalo, haloClickRipple:haloClickRipple, hidePointerHalo:hidePointerHalo, renderVideoPreview:renderVideoPreview, renderVideoDenied:renderVideoDenied, renderVideoUnsupported:renderVideoUnsupported, renderVideoRecovery:renderVideoRecovery, videoStatus:document.getElementById('bugaputa-video-status'), videoPane:document.getElementById('bugaputa-video-pane')}); if(ov) ov.style.display='none'; }catch(_){} } else { try{ cleanupVideoAttachment(); }catch(_){} var vs=document.getElementById('bugaputa-video-status'); if(vs){ vs.style.display='block'; vs.style.color='#475569'; vs.textContent='Record again from the video workspace.'; }}
});
var vRemoveBtn=h('button',{type:'button',text:'Remove',id:'bugaputa-video-remove'}); vRemoveBtn.setAttribute('aria-label','Remove recording'); vRemoveBtn.style.minHeight='44px'; vRemoveBtn.style.color='#b91c1c'; vRemoveBtn.addEventListener('click', function(){ try{ cleanupVideoAttachment(); }catch(_){} try{ Array.from(preview.children).forEach(function(ch){ if(ch!==removeBtn) ch.remove(); }); preview.appendChild(removeBtn); removeBtn.style.display='none'; var lb=document.getElementById('bugaputa-file-label'); if(lb&&lb.firstChild) lb.firstChild.textContent='Attach screenshot (optional)'; var an=document.getElementById('bugaputa-attach-notice'); if(an) an.style.display='none'; }catch(_){} });
vActions.appendChild(vPreviewBtn); vActions.appendChild(vReplaceBtn); vActions.appendChild(vRemoveBtn);
preview.appendChild(vActions);
...
var success=h('div',{id:'bugaputa-success',style:'display:none'}); success.innerHTML='<p>Thanks! Report sent.</p><p style="font-size:13px;color:#64748b;margin-top:4px">We will look into it.</p>'; success.setAttribute('role','status'); success.setAttribute('aria-live','polite');
...
var attachNotice=h('div',{id:'bugaputa-attach-notice',role:'status','aria-live':'polite',style:'display:none;margin-top:10px;padding:8px 10px;border-radius:8px;background:#ecfdf5;border:1px solid #a7f3d0;color:#065f46;font-size:12px'});
form.appendChild(msgLabel); form.appendChild(emailLabel); form.appendChild(fileLabel); form.appendChild(hpWrap); form.appendChild(ctx); form.appendChild(consent); form.appendChild(attachNotice); form.appendChild(errBox);
...
setTimeout(function(){
textarea.focus();
if(pendingVideoFile){ try{ var an2=document.getElementById('bugaputa-attach-notice'); if(an2){ an2.style.display='block'; an2.textContent='Video attached. Submit the report to send it.'; }}catch(_){} }
}, 50);
...
if(hasVideo) success.innerHTML='<p>Bug report sent</p><p style="font-size:13px;color:#475569;margin-top:4px">Video included · '+fmtVideoSize(hasVideo.size)+' · '+(pendingVideoMeta&&pendingVideoMeta.mime||hasVideo.type||"video")+'</p><p style="font-size:12px;color:#64748b;margin-top:6px">We will look into it.</p>'; else success.innerHTML='<p>Thanks! Report sent.</p><p style="font-size:13px;color:#64748b;margin-top:4px">We will look into it.</p>';
function onSuccess(){ try{ var _fa2=document.getElementById('bugaputa-actions'); if(_fa2) _fa2.style.display='none'; }catch(_){} form.style.display='none'; success.style.display='block'; setTimeout(close, 2600); }
