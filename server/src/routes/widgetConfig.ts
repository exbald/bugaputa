...
router.get("/", (req, res) => {
  ...
  try {
    const db = getDb();
    row = db.prepare("SELECT widget_label, widget_color, widget_position, videoCaptureEnabled FROM projects WHERE publicKey = ?").get(projectKey) as any;
  } catch (err) {
    console.error("[widget-config] DB error:", err);
    res.status(500).json({ error: "Internal server error" });
    return;
  }
  const videoCaptureEnabled = !!row?.videoCaptureEnabled;
  if (!row) {
