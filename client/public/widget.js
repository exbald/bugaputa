(function(){
  ...
  // Capture eval: primary = native getDisplayMedia current-tab (pixel-perfect compositor capture) when available + explicit user gesture; fallback = html2canvas 1.4.1 MIT (DOM re-render, labeled Approximate).
  //   html2canvas vs modern-screenshot: html2canvas handles broad CSS/DOM edge cases; modern-screenshot smaller but less battle-tested.
  //   Decision: native first (exact pixels), html2canvas fallback labeled Approximate, custom canvas/SVG for annotations.
  //   Rationale: html2canvas cannot guarantee pixel-identical output (flex/grid/fonts/transforms/subpixel); native compositor capture is the architectural fix.
  //   Bundle: core IIFE ~6KB gz, lazy chunk html2canvas.min.js ~45KB gz + annotate ~9KB gz incremental <90KB.
  ...
  function open(){
    ...
    consent.innerHTML='<strong style="display:block;font-size:13px;margin-bottom:6px">Before you capture</strong><p style="font-size:12px;color:#475569;line-height:1.5">Pixel-perfect capture: we\'ll ask you to pick <b>Current Tab</b> and press Share — that gives an exact viewport image. If your browser can\'t do that, we fall back to an approximate capture (labeled) or you can upload an image manually. Cross-origin iframes or protected video may appear blank. No passwords or form values are collected.</p><p style="font-size:11px;color:#94a3b8;margin-top:8px">You\'ll see a preview to confirm before annotating. Approximate captures are labeled \'Approximate capture — may differ from screen pixels\'.</p>';
  ...
  function supportsPreferCurrentTab(){
    try{
      var c=navigator.mediaDevices && navigator.mediaDevices.getSupportedConstraints ? navigator.mediaDevices.getSupportedConstraints() : {};
      return !!c.preferCurrentTab || !!c.displaySurface;
    }catch(_){ return false; }
  }
  function doNativeCapture(statusEl, formWrap, chooser, capturePane, btn, prevBtnDisplay, prevOverlayDisplay, onFallback){
    var vw=window.innerWidth, vh=window.innerHeight;
    var dpr=window.devicePixelRatio||1;
    var capDpr=Math.min(dpr,2);
    statusEl.textContent='Please select “Current Tab” and press Share…';
    statusEl.style.color='#0f172a';
    var constraints={ video:{ displaySurface:'browser' }, preferCurrentTab:true, selfBrowserSurface:'include', surfaceSwitching:'include' };
    // also try alternate shape for older Chrome
    var p=navigator.mediaDevices.getDisplayMedia(constraints).catch(function(err){
      // retry without surfaceSwitching if InvalidState
      if(err && /surfaceSwitching|selfBrowserSurface/i.test(err.message||'')){
        return navigator.mediaDevices.getDisplayMedia({ video:{ displaySurface:'browser' }, preferCurrentTab:true, selfBrowserSurface:'include' });
      }
      throw err;
    });
    p.then(function(stream){
      var track=stream.getVideoTracks()[0];
      // validate displaySurface is current-tab (browser) when exposed
      try{
        var s=track && track.getSettings ? track.getSettings() : null;
        var surf=s && s.displaySurface;
        if(surf && surf!=='browser'){
          // wrong surface (window/screen) — reject with mismatch so choice UI shows
          try{ stream.getTracks().forEach(function(t){ try{t.stop();}catch(_){}}); }catch(_){}
          onFallback('native-dimensions-mismatch displaySurface='+surf);
          return;
        }
      }catch(_){}
      ...
      function tryCaptureFrame(){
        ...
        var expectedW=Math.round(vw*capDpr), expectedH=Math.round(vh*capDpr);
        var expectedW1=Math.round(vw*dpr), expectedH1=Math.round(vh*dpr);
        var wMatch=Math.abs(vW-expectedW)<=2 && Math.abs(vH-expectedH)<=2;
        var wMatch1=Math.abs(vW-expectedW1)<=2 && Math.abs(vH-expectedH1)<=2;
        var useDpr=wMatch?capDpr:(wMatch1?dpr:0);
        if(!wMatch && !wMatch1){
          // frame includes chrome or is screen/window — do not attempt unreliable crop
          fallback('native-dimensions-mismatch '+vW+'x'+vH+' expected '+expectedW+'x'+expectedH+'/'+expectedW1+'x'+expectedH1+' outer '+window.outerWidth+'x'+window.outerHeight+' inner '+vw+'x'+vh+' DPR '+dpr);
          return;
        }
        var canvas=document.createElement('canvas');
        canvas.width=Math.round(vw*useDpr);
        canvas.height=Math.round(vh*useDpr);
        var ctx=canvas.getContext('2d');
        try{
          ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
        }catch(e){ fallback('native-drawImage '+e.message); return; }
        var dataUrl;
        try{ dataUrl=canvas.toDataURL('image/png'); }catch(e){ fallback('native-tainted '+e.message); return; }
        if(!dataUrl || dataUrl==='data:,'){ fallback('native-blank'); return; }
        successCanvas(canvas, useDpr, dataUrl);
      }
      video.addEventListener('loadedmetadata', function(){
        // give browser a frame
        setTimeout(tryCaptureFrame, 120);
        // also try after play
      ...
      if(name==='NotAllowedError' || /Permission/i.test(msg)) onFallback('permission-denied');
      else if(name==='NotFoundError') onFallback('no-source');
      else if(name==='AbortError') onFallback('aborted');
      else onFallback(msg.slice(0,120));
    });
  }
  // ---------- capture ----------
  function doCapture(statusEl, formWrap, chooser, capturePane){
    ...
    function runCaptureFallback(labelAsApproximate){
      ...
      setTimeout(function(){
        try{
          ...
          hc(document.body, opts).then(function(canvas){
            ...
            statusEl.style.display='none';
            capturedDataUrl=dataUrl;
            capturedDims={w:canvas.width, h:canvas.height, cssW: vw, cssH: vh, dpr: opts.scale, approximate: !!labelAsApproximate, native: false};
    ...
    function showNativeChoice(statusEl, formWrap, chooser, capturePane, btn, prevBtnDisplay, prevOverlayDisplay, reason){
      // stop any tracks/video already cleaned by doNativeCapture's fallback path
      var msg='Native capture unavailable.';
      if(reason==='permission-denied') msg='Permission denied — browser did not allow screen capture.';
      else if(reason==='aborted') msg='Capture was cancelled.';
      else if(reason && reason.indexOf('native-dimensions-mismatch')===0) msg='Browser capture included window chrome (wrong surface).';
      else if(reason) msg='Native capture failed: '+String(reason).slice(0,120);
      statusEl.style.color='#dc2626';
      statusEl.textContent=msg;
      // remove any prior choice row
      var prev=document.getElementById('bugaputa-native-choice');
      if(prev) prev.remove();
      var row=h('div',{id:'bugaputa-native-choice',style:'display:flex;flex-wrap:wrap;gap:8px;margin-top:10px'});
      var btnRetry=h('button',{type:'button',text:'Retry native (Current Tab)'});
      btnRetry.setAttribute('aria-label','Retry native capture');
      btnRetry.addEventListener('click', function(){ row.remove(); doCapture(statusEl, formWrap, chooser, capturePane); });
      var btnApprox=h('button',{type:'button',text:'Use approximate capture'});
      btnApprox.title='Approximate capture — may differ from screen pixels';
      btnApprox.setAttribute('aria-label','Use approximate capture');
      btnApprox.addEventListener('click', function(){
        row.remove();
        statusEl.textContent='Creating approximate capture\u2026';
        statusEl.style.color='#475569';
        ensureHtml2Canvas(function(){ runCaptureFallback(true); });
      });
      var btnUpload=h('button',{type:'button',text:'Upload image'});
      ...
      var btnCancel=h('button',{type:'button',text:'Cancel'});
      ...
      row.appendChild(btnRetry); row.appendChild(btnApprox); row.appendChild(btnUpload); row.appendChild(btnCancel);
    ...
    if(isNativeCaptureSupported()){
      doNativeCapture(statusEl, formWrap, chooser, capturePane, btn, prevBtnDisplay, prevOverlayDisplay, function(reason){
        console.warn('[Bugaputa] native capture fallback:', reason);
  ...
  function openAnnotateEditor(blobUrl, dataUrl, capCanvas, formWrap, chooser, capturePane){
    ...
    // fit wrapper to viewport without upscale, observe resize
    var _applyFitCleanup=null, _onWinResize=null;
    function applyFit(){
      var maxW=stage.clientWidth-32, maxH=stage.clientHeight-32;
      if(!maxW||!maxH) return;
      var scale=Math.min(1, maxW/capturedDims.cssW, maxH/capturedDims.cssH);
      canvasWrap.style.width=Math.round(capturedDims.cssW*scale)+'px';
      canvasWrap.style.height=Math.round(capturedDims.cssH*scale)+'px';
    }
    applyFit();
    _onWinResize=applyFit;
    window.addEventListener('resize', _onWinResize);
    var _ro=null;
    try{ _ro=new ResizeObserver(applyFit); _ro.observe(stage); }catch(_){}
    _applyFitCleanup=function(){ try{ window.removeEventListener('resize', _onWinResize);}catch(_){} try{ if(_ro) _ro.disconnect(); }catch(_){}}
    // bottom pill toolbar
    ...
    ed._cleanup=function(){ document.removeEventListener('keydown', edTrap); document.body.style.overflow=''; try{ if(_applyFitCleanup) _applyFitCleanup(); }catch(_){} };
    // canvas drawing
    ...
    function cssPoint(e){
      ...
      var sx=capturedDims.cssW / rect.width, sy=capturedDims.cssH / rect.height;
