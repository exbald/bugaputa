...
var _svgP='<svg class="bugaputa-live-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">',_svgS='</svg>';var _ico={select:_svgP+'<path d="M5 3l14 9-7 1-3 7z"/><path d="M12 13l-3 7"/>'+_svgS,pen:_svgP+'<path d="M17 3a2.8 2.8 0 0 1 4 4L7 21l-4 1 1-4z"/><path d="M15 5l4 4"/>'+_svgS,arrow:_svgP+'<path d="M5 12h14"/><path d="M13 5l7 7-7 7"/>'+_svgS,rect:_svgP+'<rect x="3" y="3" width="18" height="18" rx="2"/>'+_svgS,text:_svgP+'<path d="M4 7V5a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v2"/><path d="M9 20h6"/><path d="M12 4v16"/>'+_svgS};
...
tools.forEach(function(id){ var icon=_ico[id]; var label=_labels[id]; var html=icon; if(id==='text') html=icon+'<span class="bugaputa-live-label">Text</span>'; var b=h('button',{type:'button','aria-label':label,title:label,html:html}); b.dataset.tool=id; if(id==='text'){ b.classList.add('bugaputa-live-labeled'); b.style.cssText='min-height:44px;min-width:44px;flex:0 0 auto;white-space:nowrap'; } else { b.style.cssText='min-width:44px;min-height:44px'; } b.addEventListener('click',function(){ __liveState.tool=id; Object.keys(btns).forEach(function(k){ var on=k===id; btns[k].setAttribute('aria-pressed',on?'true':'false'); }); cvs.style.cursor=id==='select'?'default':'crosshair'; updatePointer(); }); btns[id]=b; tb.appendChild(b); });
...
[undo,redo,del,clr].forEach(function(b){ b.style.cssText='min-height:44px;min-width:44px;flex:0 0 auto;white-space:nowrap'; });
...
function updateUndoRedoDisabled(){ try{ undo.disabled=!__liveState.undoStack.length; redo.disabled=!__liveState.redoStack.length; undo.setAttribute('aria-disabled',undo.disabled?'true':'false'); redo.setAttribute('aria-disabled',redo.disabled?'true':'false'); }catch(_){} }
