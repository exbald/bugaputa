...
/* Privacy: current-tab-only (Capture Handle). Hints alone cannot legally constrain. See spike doc. */
...
if(__bugaputaCaptureToken && !ht) return {ok:false, reason:'handle missing'};
...
onDenied('Mic unavailable — retry');
return;
}
...
// Live workspace must NOT hide body overflow — restores overlay._prevOverflow so wheel/trackpad/touch scroll works
...
var _svgP='<svg class="bugaputa-live-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">',_svgS='</svg>';var _ico={select:_svgP+'<path d="M5 3l14 9-7 1-3 7z"/><path d="M12 13l-3 7"/>'+_svgS,interact:_svgP+'<path d="M12 8v6"/><path d="M8 13l4 3 4-3"/><path d="M8 9h8"/>'+_svgS,pen:_svgP+'<path d="M17 3a2.8 2.8 0 0 1 4 4L7 21l-4 1 1-4z"/><path d="M15 5l4 4"/>'+_svgS,arrow:_svgP+'<path d="M5 12h14"/><path d="M13 5l7 7-7 7"/>'+_svgS,rect:_svgP+'<rect x="3" y="3" width="18" height="18" rx="2"/>'+_svgS,text:_svgP+'<path d="M4 7V5a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v2"/><path d="M9 20h6"/><path d="M12 4v16"/>'+_svgS};
var _labels={select:'Select and move annotations',interact:'Interact with page',pen:'Pen',arrow:'Arrow',rect:'Rect',text:'Text'};
function _sep(){var s=document.createElement('span');s.className='bugaputa-live-sep';s.setAttribute('aria-hidden','true');return s}
var tools=['select','interact','pen','arrow','rect','text'], btns={};
tools.forEach(function(id){var icon=_ico[id],label=_labels[id],html=icon;if(id==='text'||id==='select'||id==='interact')html=icon+'<span class="bugaputa-live-label">'+(id==='select'?'Select':id==='interact'?'Interact':'Text')+'</span>';var b=h('button',{type:'button','aria-label':label,title:label,html:html});b.dataset.tool=id;if(id==='text'||id==='select'||id==='interact')b.classList.add('bugaputa-live-labeled');b.style.cssText='flex:0 0 auto;white-space:nowrap';b.addEventListener('click',function(){__liveState.tool=id;Object.keys(btns).forEach(function(k){btns[k].setAttribute('aria-pressed',k===id?'true':'false');});updatePointer();});btns[id]=b;tb.appendChild(b);});
var undo=h('button',{type:'button','aria-label':'Undo',title:'Undo (Ctrl+Z)',html:_svgP+'<path d="M9 14 4 9l5-5"/><path d="M4 9h10.5a5.5 5.5 0 0 1 5.5 5.5a5.5 5.5 0 0 1-5.5 5.5H11"/>'+_svgS+'<span class="bugaputa-live-label">Undo</span>'}), redo=h('button',{type:'button','aria-label':'Redo',title:'Redo (Ctrl+Shift+Z)',html:_svgP+'<path d="m15 14 5-5-5-5"/><path d="M20 9H9.5A5.5 5.5 0 0 0 4 14.5A5.5 5.5 0 0 0 9.5 20H13"/>'+_svgS+'<span class="bugaputa-live-label">Redo</span>'}), del=h('button',{type:'button','aria-label':'Delete selected',title:'Delete selected',html:_svgP+'<path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/>'+_svgS}), clr=h('button',{type:'button','aria-label':'Clear all annotations',title:'Clear all annotations',html:_svgP+'<path d="M21 21H8a2 2 0 0 1-1.42-.587l-3.994-3.999a2 2 0 0 1 0-2.828l10-10a2 2 0 0 1 2.829 0l5.999 6a2 2 0 0 1 0 2.828L12.834 21"/><path d="m5.082 11.09 8.828 8.828"/>'+_svgS+'<span class="bugaputa-live-label">Clear all</span>'}), done=h('button',{type:'button','aria-label':'Done',title:'Done',html:'Done'}), cancel=h('button',{type:'button','aria-label':'Cancel',title:'Cancel',html:'Cancel'});
...
undo.classList.add('bugaputa-live-labeled'); redo.classList.add('bugaputa-live-labeled'); clr.classList.add('bugaputa-live-labeled');
[undo,redo,del,clr].forEach(function(b){ b.style.cssText='flex:0 0 auto;white-space:nowrap'; });
if(undo.classList.contains('bugaputa-live-labeled')) undo.style.padding='0 8px';
if(redo.classList.contains('bugaputa-live-labeled')) redo.style.padding='0 8px';
if(clr.classList.contains('bugaputa-live-labeled')) clr.style.padding='0 8px';
done.style.cssText='min-width:44px;min-height:44px;flex:0 0 auto'; cancel.style.cssText='min-width:44px;min-height:44px;flex:0 0 auto';
...
function pushU(){__liveState.undoStack.push(JSON.stringify(__liveState.annotations));if(__liveState.undoStack.length>40)__liveState.undoStack.shift();__liveState.redoStack=[];updateUndoRedoDisabled();}
function doUndo(){if(!__liveState.undoStack.length)return;__liveState.redoStack.push(JSON.stringify(__liveState.annotations));__liveState.annotations=JSON.parse(__liveState.undoStack.pop());__liveState.selectedId=null;draw();updateUndoRedoDisabled();}
function doRedo(){if(!__liveState.redoStack.length)return;__liveState.undoStack.push(JSON.stringify(__liveState.annotations));__liveState.annotations=JSON.parse(__liveState.redoStack.pop());__liveState.selectedId=null;draw();updateUndoRedoDisabled();}
function _ud(){try{var has=!!__liveState.selectedId;del.disabled=!has;del.setAttribute('aria-disabled',has?'false':'true');}catch(_){}}function updateUndoRedoDisabled(){try{undo.disabled=!__liveState.undoStack.length;redo.disabled=!__liveState.redoStack.length;undo.setAttribute('aria-disabled',undo.disabled?'true':'false');redo.setAttribute('aria-disabled',redo.disabled?'true':'false');_ud();}catch(_){}}
updateUndoRedoDisabled();
undo.addEventListener('click',doUndo); redo.addEventListener('click',doRedo); del.addEventListener('click',function(){if(!__liveState.selectedId)return;pushU();__liveState.annotations=__liveState.annotations.filter(function(a){return a.id!==__liveState.selectedId;});__liveState.selectedId=null;draw();updateUndoRedoDisabled();}); clr.addEventListener('click',function(){ if(!__liveState.annotations.length) return; if(!confirm('Clear all annotations?')) return; pushU(); __liveState.annotations=[]; __liveState.selectedId=null; draw(); });
function _d(ax,ay,bx,by,px,py){var vx=bx-ax,vy=by-ay,wx=px-ax,wy=py-ay,c1=vx*wx+vy*wy;if(c1<=0)return Math.hypot(px-ax,py-ay);var c2=vx*vx+vy*vy;if(c2<=c1)return Math.hypot(px-bx,py-by);var t=c1/c2;return Math.hypot(px-(ax+t*vx),py-(ay+t*vy));}
function _h(a,p){var x=p.x,y=p.y;if(a.type==='rect'){var x1=Math.min(a.x,a.x2),x2=Math.max(a.x,a.x2),y1=Math.min(a.y,a.y2),y2=Math.max(a.y,a.y2);return x>=x1-10&&x<=x2+10&&y>=y1-10&&y<=y2+10;}if(a.type==='arrow')return _d(a.x,a.y,a.x2,a.y2,x,y)<=12;if(a.type==='pen'){for(var i=1;i<a.points.length;i++)if(_d(a.points[i-1][0],a.points[i-1][1],a.points[i][0],a.points[i][1],x,y)<=12)return true;return a.points.length&&Math.hypot(a.points[0][0]-x,a.points[0][1]-y)<=12;}if(a.type==='text'){var w=(a.text||'').length*7+12;return x>=a.x-6&&x<=a.x+w&&y>=a.y-14&&y<=a.y+6;}return false;}
var ctx=cvs.getContext('2d'), raf=0; function draw(){ if(raf) return; raf=requestAnimationFrame(function(){ raf=0; ctx.clearRect(0,0,W,H); __liveState.annotations.forEach(function(a){ ctx.save(); ctx.strokeStyle=a.color; ctx.fillStyle=a.color; ctx.lineWidth=2.5; ctx.lineCap='round'; ctx.lineJoin='round'; if(a.type==='rect') ctx.strokeRect(Math.min(a.x,a.x2),Math.min(a.y,a.y2),Math.abs(a.x2-a.x),Math.abs(a.y2-a.y)); else if(a.type==='arrow'){ ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(a.x2,a.y2); ctx.stroke(); var ang=Math.atan2(a.y2-a.y,a.x2-a.x),L=14; ctx.beginPath(); ctx.moveTo(a.x2,a.y2); ctx.lineTo(a.x2-L*Math.cos(ang-Math.PI/6),a.y2-L*Math.sin(ang-Math.PI/6)); ctx.lineTo(a.x2-L*Math.cos(ang+Math.PI/6),a.y2-L*Math.sin(ang+Math.PI/6)); ctx.closePath(); ctx.fill(); } else if(a.type==='pen'&&a.points.length>=2){ ctx.beginPath(); ctx.moveTo(a.points[0][0],a.points[0][1]); for(var i=1;i<a.points.length;i++) ctx.lineTo(a.points[i][0],a.points[i][1]); ctx.stroke(); } else if(a.type==='text'){ ctx.font='14px Inter,system-ui'; ctx.fillText(a.text||'',a.x,a.y); } ctx.restore(); }); if(__liveState.selectedId){var o=null;for(var k=0;k<__liveState.annotations.length;k++)if(__liveState.annotations[k].id===__liveState.selectedId)o=__liveState.annotations[k];if(o){var x1,y1,x2,y2;if(o.type==='rect'||o.type==='arrow'){x1=Math.min(o.x,o.x2);y1=Math.min(o.y,o.y2);x2=Math.max(o.x,o.x2);y2=Math.max(o.y,o.y2);}else if(o.type==='pen'){x1=x2=o.points[0][0];y1=y2=o.points[0][1];}else{x1=o.x-2;y1=o.y-14;x2=o.x+((o.text||'').length*7+8);y2=o.y+4;}if(x1!=null){ctx.save();ctx.strokeStyle='#0f172a';ctx.setLineDash([5,3]);ctx.strokeRect(x1-4,y1-4,x2-x1+8,y2-y1+8);ctx.setLineDash([]);ctx.restore();}}} }); }

var onScroll=function(){ draw(); }; window.addEventListener('scroll',onScroll,{passive:true});
function updatePointer(){var i=__liveState.tool==='interact';cvs.style.pointerEvents=i?'none':'auto';cvs.style.touchAction=i?'pan-x pan-y':'none';cvs.style.cursor=i?'grab':(__liveState.tool==='select'?'default':'crosshair');}
updatePointer();var drawing=null,isDown=false,dg=null,dS=null,dO=null,uS=null;function _o(a){if(a.type==='rect'||a.type==='arrow')return{x:a.x,y:a.y,x2:a.x2,y2:a.y2};if(a.type==='pen')return{pts:a.points.map(function(p){return[p[0],p[1]]})};if(a.type==='text')return{x:a.x,y:a.y};return{}}function _a(a,dx,dy){if(a.type==='rect'||a.type==='arrow'){a.x=dO.x+dx;a.y=dO.y+dy;a.x2=dO.x2+dx;a.y2=dO.y2+dy;}else if(a.type==='pen'){for(var i=0;i<a.points.length;i++){a.points[i][0]=dO.pts[i][0]+dx;a.points[i][1]=dO.pts[i][1]+dy;}}else if(a.type==='text'){a.x=dO.x+dx;a.y=dO.y+dy;}}
cvs.addEventListener('click',function(e){if(__liveState.tool!=='interact'){e.preventDefault();e.stopPropagation();}});
cvs.addEventListener('pointerdown',function(e){if(__liveState.tool==='interact')return;e.preventDefault();e.stopPropagation();var pt=getDocPoint(e);if(__liveState.tool==='select'){var hit=null;for(var i=__liveState.annotations.length-1;i>=0;i--){if(_h(__liveState.annotations[i],pt)){hit=__liveState.annotations[i];break;}}if(hit){if(__liveState.selectedId!==hit.id){__liveState.selectedId=hit.id;draw();}dg=hit;dS=pt;dO=_o(hit);uS=JSON.stringify(__liveState.annotations);try{cvs.setPointerCapture(e.pointerId);}catch(_){}updateUndoRedoDisabled();}else{if(__liveState.selectedId){__liveState.selectedId=null;draw();updateUndoRedoDisabled();}dg=null;dS=null;}return;}if(__liveState.tool==='text'){var t=prompt('Enter text:','');if(t===null)return;t=String(t).slice(0,200);if(!t.trim())return;pushU();__liveState.annotations.push({id:'a_'+Math.random().toString(36).slice(2,9),type:'text',x:pt.x,y:pt.y,text:t,color:__liveState.color});__liveState.selectedId=__liveState.annotations[__liveState.annotations.length-1].id;draw();updateUndoRedoDisabled();return;}var isDraw=__liveState.tool==='pen'||__liveState.tool==='rect'||__liveState.tool==='arrow';if(!isDraw)return;pushU();isDown=true;try{cvs.setPointerCapture(e.pointerId);}catch(_){}if(__liveState.tool==='pen')drawing={id:'a_'+Math.random().toString(36).slice(2,9),type:'pen',color:__liveState.color,points:[[pt.x,pt.y]]};else if(__liveState.tool==='rect')drawing={id:'a_'+Math.random().toString(36).slice(2,9),type:'rect',color:__liveState.color,x:pt.x,y:pt.y,x2:pt.x,y2:pt.y};else if(__liveState.tool==='arrow')drawing={id:'a_'+Math.random().toString(36).slice(2,9),type:'arrow',color:__liveState.color,x:pt.x,y:pt.y,x2:pt.x,y2:pt.y};if(drawing)__liveState.annotations.push(drawing);draw();});
cvs.addEventListener('pointermove',function(e){if(dg&&dS){e.preventDefault();var pt=getDocPoint(e);var dx=pt.x-dS.x,dy=pt.y-dS.y;_a(dg,dx,dy);draw();return;}if(!isDown||!drawing)return;e.preventDefault();var pt2=getDocPoint(e);if(drawing.type==='pen')drawing.points.push([pt2.x,pt2.y]);else{drawing.x2=pt2.x;drawing.y2=pt2.y;}draw();});
function endLive(e){if(dg&&dS){var pt=getDocPoint(e);var dx=pt.x-dS.x,dy=pt.y-dS.y;if(Math.hypot(dx,dy)>1){__liveState.undoStack.push(uS);if(__liveState.undoStack.length>40)__liveState.undoStack.shift();__liveState.redoStack=[];updateUndoRedoDisabled();}dg=null;dS=null;dO=null;uS=null;draw();try{cvs.releasePointerCapture(e.pointerId);}catch(_){}e.preventDefault();e.stopPropagation();return;}if(!isDown)return;isDown=false;if(drawing&&((drawing.type==='rect'||drawing.type==='arrow')&&Math.hypot(drawing.x2-drawing.x,drawing.y2-drawing.y)<6))__liveState.annotations=__liveState.annotations.filter(function(a){return a.id!==drawing.id;});else if(drawing)__liveState.selectedId=drawing.id;drawing=null;draw();updateUndoRedoDisabled();try{cvs.releasePointerCapture(e.pointerId);}catch(_){}e.preventDefault();e.stopPropagation();}
