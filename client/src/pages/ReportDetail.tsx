  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="max-w-4xl mx-auto w-full px-4 py-6 flex-1">
        ...
        {lightbox && imgSrc && (
          <div role="dialog" aria-modal="true" aria-label="Screenshot full size" onClick={()=> setLightbox(false)} className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 pt-[calc(16px+env(safe-area-inset-top,0px))] pb-[calc(16px+env(safe-area-inset-bottom,0px))] cursor-zoom-out" onKeyDown={e=>{ if(e.key==="Escape") setLightbox(false); if(e.key==="Tab"){ const f=(e.currentTarget as HTMLElement).querySelectorAll("button, a[href]"); if(!f.length) return; const first=f[0] as HTMLElement, last=f[f.length-1] as HTMLElement; if(e.shiftKey && document.activeElement===first){ e.preventDefault(); last.focus(); } else if(!e.shiftKey && document.activeElement===last){ e.preventDefault(); first.focus(); } } }} tabIndex={-1} autoFocus>
