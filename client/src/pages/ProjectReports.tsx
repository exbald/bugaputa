...
export default function ProjectReports(){
  ...
  const snippet=pk?'<scr'+'ipt src="https://bugaputa.no-code.gdn/widget.js" data-project="'+pk+'" data-label="'+wLabel.replace(/"/g,'&quot;')+'" data-color="'+wColor+'" data-position="'+wPos+'"></scr'+'ipt>':"";
      ...
      <main className="max-w-6xl mx-auto w-full px-4 py-6 flex-1">
          ...
          <div className="mt-3 bg-white border rounded-2xl p-5">
            ...
            {snippet && (
              <div className="mt-3 bg-slate-50 border rounded-xl p-3">
                ...
                <button onClick={handleCopySnippet} className="mt-2 text-xs px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800">{copiedSnippet ? "Copied!" : "Copy snippet"}</button>
              </div>
            )}
          </div>
        )}
        {project && (
          <div className="mt-6 bg-white border rounded-2xl p-5">
            ...
            <p className="text-xs text-slate-500 mt-1">Customize the feedback tab label, color, and position.</p>
            <div className="mt-4 grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <label htmlFor="widget-label" className="block text-xs font-semibold text-slate-700 mb-1.5">Tab label</label>
                  <input id="widget-label" type="text" maxLength={30} value={wLabel} onChange={e=> setWLabel(e.target.value)} placeholder="Feedback" className="w-full border rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-lime-500" />
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <input id="widget-color-hex" type="text" value={wColor} onChange={e=> setWColor(e.target.value)} placeholder="#171717" pattern="^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$" className="flex-1 border rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-lime-500 font-mono" />
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {WIDGET_POSITIONS.map(opt=>(
                      <button key={opt.value} type="button" onClick={()=> setWPos(opt.value)} className={'px-3 py-2.5 rounded-xl text-sm font-medium border transition text-center '+(wPos===opt.value ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50")}>{opt.label}</button>
                    ))}
                  </div>
                </div>
                <div className="flex items-center gap-3 pt-1">
                  <button onClick={handleSaveWidget} disabled={wSaving} className="px-4 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 disabled:opacity-50 min-h-[40px]">{wSaving ? "Saving..." : "Save widget settings"}</button>
